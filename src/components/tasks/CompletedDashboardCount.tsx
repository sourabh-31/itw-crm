...
export default function CompletedDashboardCount() {
  const { timeFilter } = useTaskStore();

  const { data } = useTaskStats("COMPLETED", timeFilter, 0);
    ...
    >
      ...
      <div className="flex items-end gap-1">
        <div className="flex w-[165px] flex-col items-center gap-[10px]">
          ...
          <div className="font-recoletaAlt text-4xl text-[#171F25]">
            {data?.data?.completedTasks?.totalCount ?? 0}
          </div>
          <div className="relative left-[6px] mt-[-6px] flex w-3/5 items-start gap-[2px]">
            {data &&
              data?.data.completedTasks.changeFromLastNDaysBeforeThat !==
                "NO_CHANGE" && (
                <Image
                  src={
                    data?.data.completedTasks.changeFromLastNDaysBeforeThat ===
            ...
            {data?.data.completedTasks.changeFromLastNDaysBeforeThat ===
              "INCREASE" && (
              <span className="font-mulish text-xs font-semibold text-[#0EAB00]">
                +{data.data.completedTasks.changeNumber} more than last week
              </span>
            )}
            {data?.data.completedTasks.changeFromLastNDaysBeforeThat ===
              "DECREASE" && (
              <span className="font-mulish text-xs font-semibold text-[#FF6161]">
                -{data.data.completedTasks.changeNumber} fewer than last week
              </span>
            )}
          </div>
        </div>
